(function(o,i,k){
  i.infinitescroll=function z(D,F,E){
    this.element=i(E);if(!this._create(D,F)){this.failed=true}};i.infinitescroll.defaults={
      loading:{
      finished:k,finishedMsg:"<em>Congratulations, you've reached the end of the internet.</em>",
...
i.infinitescroll.prototype={
  ...
  if(F==="unbind"){
    ...
    (this.options.binder)[F]("smartscroll.infscr."+D.options.infid,
    function(){
      D.scroll()
    })
  }
...
_create:function t(F,J){
  ...
  G.path=G.path||this._determinepath(H);G.contentSelector=G.contentSelector||this.element;G.loading.selector=G.loading.selector||G.contentSelector;G.loading.msg=G.loading.msg||i('<div id="infscr-loading"><img alt="Loading..." src="'+G.loading.img+'" /><div>'+G.loading.msgText+"</div></div>");
  ...
  var E=this;G.loading.start=G.loading.start||function(){i(G.navSelector).hide();
      ...
      if(E.match(/^(.*?)\b2\b(.*?$)/)){
        ...
        if(E.match(/^(.*?)2(.*?$)/)){
          ...
          if(E.match(/^(.*?page=)1(\/.*|$)/)){
            ...
            this._debug("Sorry, we couldn't parse your Next (Previous Posts) URL. Verify the css selector points to the correct A tag. If you still get this error: yell, scream, and kindly ask for help at infinite-scroll.com.");
  ...
  _error:function v(E){
    ...
    _loadcallback:function c(H,G,E){
      ...
      switch(L){
        ...
        this._showdonemsg();
        return false;
        case"no-append":if(D.dataType==="html"){
          G="<div>"+G+"</div>";
        ...
        if(D.animate){
          var I=i(o).scrollTop()+i("#infscr-loading").height()+D.extraScrollPx+"px";i("html,body").animate({scrollTop:I},800,function(){
        ...
        J(this,G,E);
        if(D.prefill){
          this._prefill()}},_nearbottom:function u(){
            var E=this.options,
          ...
          _showdonemsg:function a(){
            ...
            D.errorCallback.call(i(D.contentSelector)[0],"done")},_validate:function w(E){
              for(var D in E){
            ...
            var x=i.event,s;x.special.smartscroll={setup:function(){
              i(this).bind("scroll",x.special.smartscroll.handler)},teardown:function(){
                i(this).unbind("scroll",x.special.smartscroll.handler)},handler:function(G,D){
                  var F=this,E=arguments;G.type="smartscroll";
                  if(s){
                    clearTimeout(s)
                  }
                  s=setTimeout(function(){
                    i(F).trigger("smartscroll",E)},D==="execAsap"?0:100)
                  }
                };
